# 1) Use Debian Bookworm + CUDA 11.8 devel (with cuDNN) so libsndio6 exists
FROM nvidia/cuda:11.8.0-devel-bookworm

USER root

# 2) Install build & runtime deps, including libsndio6
...
# 4) Build NVIDIA codec headers
RUN git clone https://git.videolan.org/git/ffmpeg/nv-codec-headers.git \
 && cd nv-codec-headers && make install \
 && cd .. && rm -rf nv-codec-headers

# 5) Clone & build FFmpeg with sndio & NVENC/NVDEC
RUN git clone https://git.ffmpeg.org/ffmpeg.git ffmpeg \
 ...
 && cd .. && rm -rf ffmpeg \
 && ldconfig \
 # verify we have no missing libs
 && ffmpeg -version

# 6) Ensure CUDA & local libs are found at runtime
...
# 7) Install PyTorch cu118 + Whisper, pre-download base model
...
# 9) Prepare QNAP Container Station mounts & perms
...
# 10) Switch to unprivileged node user & expose port
USER node
EXPOSE 5678

# 11) Start n8n (CMD for easy overrides)
CMD ["n8n", "start"]

...
###############################################################################
# Stage 1 ─ build a *fully-static*, CUDA/NVENC-enabled FFmpeg (no runtime .so)
###############################################################################
...
# —————————————————————————————————————————————
# 1) Build-time tooling only (no shared libs are kept)
# —————————————————————————————————————————————
RUN apt-get update && \
...
# —————————————————————————————————————————————
# 2) NVENC headers (needed for --enable-nvenc)
# —————————————————————————————————————————————
...
# —————————————————————————————————————————————
# 3) External codec libs (static .a only)
# —————————————————————————————————————————————
...
# —————————————————————————————————————————————
# 4) Build FFmpeg (static, CUDA/NVENC)
# —————————————————————————————————————————————
...
# —————————————————————————————————————————————
# 1) Base OS libs + Chrome runtime deps
# —————————————————————————————————————————————
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
...
# —————————————————————————————————————————————
# 5) Static FFmpeg + probe (and NVIDIA bin symlinks)
# —————————————————————————————————————————————
...
# —————————————————————————————————————————————
# 7) Puppeteer-managed Chrome (download as node user)
# —————————————————————————————————————————————
USER node
...
# —————————————————————————————————————————————
# 9) Cache Whisper “medium” model (no heredoc)
# —————————————————————————————————————————————
RUN mkdir -p "${WHISPER_MODEL_PATH}" && \
    echo "import whisper, os; whisper.load_model('medium', download_root=os.environ['WHISPER_MODEL_PATH'])" \
        > /tmp/preload_whisper.py && \
    python3.10 /tmp/preload_whisper.py && \
    rm /tmp/preload_whisper.py

# —————————————————————————————————————————————
# 10) Sanity-check static FFmpeg + CUDA hwaccels
# —————————————————————————————————————————————
RUN which -a ffmpeg && \
...
# —————————————————————————————————————————————
# 11) PATH shim (static ffmpeg first after NVIDIA hook)
# —————————————————————————————————————————————
RUN printf '%s\n' \
    '#!/bin/sh' \

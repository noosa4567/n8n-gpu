###############################################################################
# Stage 1 – build FFmpeg with VAAPI + NVENC/NVDEC + sndio                     #
###############################################################################
...
RUN git clone https://git.ffmpeg.org/ffmpeg.git ffmpeg && \
    ...
    # quick sanity check: every linked .so must resolve
    ldd /usr/local/bin/ffmpeg | grep "not found" && echo "ldd OK"
...
RUN apt-get update && apt-get install -y --no-install-recommends \
      ...
      libva2 libva-x11-2 libva-drm2 libvdpau1         \
...
RUN mkdir -p /usr/local/lib/whisper_models && \
    python3 - <<'PY' \
import whisper,os; whisper.load_model("base", download_root="/usr/local/lib/whisper_models") \
PY

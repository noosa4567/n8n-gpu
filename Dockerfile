...
# 2) Install tini, pip & minimal runtime libs (plus XCB for FFmpeg)
RUN apt-get update \
...
# 3) Pull in FFmpeg binaries & libs, update cache
COPY --from=ffmpeg /usr/local/bin/ffmpeg  /usr/local/bin/
COPY --from=ffmpeg /usr/local/bin/ffprobe /usr/local/bin/
COPY --from=ffmpeg /usr/local/lib/        /usr/local/lib/
RUN ldconfig

# 4) Install Node 20 & pinned n8n CLI
...
# 10) Install Puppeteer + community node into ~/.n8n

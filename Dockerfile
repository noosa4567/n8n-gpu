...
# 1) Pull in passwd for groupadd/useradd, tini, pip, Puppeteer deps, etc.
RUN apt-get update \
...
# 2) Bring in our GPU-enabled FFmpeg binaries & libs
COPY --from=ffmpeg /usr/local/bin/ffmpeg  /usr/local/bin/
COPY --from=ffmpeg /usr/local/bin/ffprobe /usr/local/bin/
COPY --from=ffmpeg /usr/local/lib/        /usr/local/lib/
RUN ldconfig

# 3) Install Node 20, n8n CLI, Puppeteer & the community node 
...
# 4) Whisper + tokenizer + pre-download “base” model (with a retry fallback)
...
# 5) Prepare shared data dirs (tight permissions)
...
# 7) Healthcheck for n8n readiness
...
# 9) Launch n8n in default server mode under tini
ENTRYPOINT ["tini","--","n8n"]
CMD []

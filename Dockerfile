...
# ── 1) tool-chain only — runtime stays clean ─────────────────────────────────
RUN apt-get update && \
...
# ── 3) external codec libs – all static (.a) ─────────────────────────────────
...
# 8) Torch + Whisper (CPU build ok for quantisation)
...
# 9) download + quantise Whisper *medium* (CPU, int-8)
RUN mkdir -p "$WHISPER_MODEL_PATH" && \
    ...
    > /tmp/quantise.py && \
    python3.10 /tmp/quantise.py && rm /tmp/quantise.py

# 9b) cache symlink so load_model finds it
RUN mkdir -p /home/node/.cache && \
    ln -s /usr/local/lib/whisper_models /home/node/.cache/whisper && \
    chown -h node:node /home/node/.cache/whisper

# 10) sanity-check FFmpeg

...
RUN git clone https://git.ffmpeg.org/ffmpeg.git ffmpeg \
  ...
  && LD_LIBRARY_PATH=/usr/local/lib:/usr/local/cuda/lib64:/usr/local/nvidia/lib \
     ldd /usr/local/bin/ffmpeg | grep -q "not found" \
       ...
       || echo "✅ FFmpeg OK" \
  && echo "📄 FFmpeg version & configuration:" \
  && /usr/local/bin/ffmpeg -version \
  && echo "🔢 Encoders:" && /usr/local/bin/ffmpeg -encoders \
  && echo "🔢 Decoders:" && /usr/local/bin/ffmpeg -decoders \
  && cd .. && rm -rf ffmpeg

# ┌─────────────────────────────────────────────────────────────────────────────┐
# │ Stage 2: Runtime (aligned CUDA 11.8 PyTorch runtime + n8n + Whisper)        │
# └─────────────────────────────────────────────────────────────────────────────┘

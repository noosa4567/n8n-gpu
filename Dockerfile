...
###############################################################################
# Stage 2 • Runtime: CUDA 11.8 Ubuntu 22.04 + PyTorch, Whisper, n8n & Puppeteer
###############################################################################
...
# 1) Create non-root "node" user with UID 999
RUN useradd -m -u 999 -s /bin/bash node

# 2) Install system deps (with retry to dodge NVidia mirror sync issues)
...
# 3) Copy FFmpeg & libs from builder
...
# 5) Globally install n8n, Puppeteer (bundled Chromium), community node & ajv peer
RUN npm install -g \
      n8n@latest \
      puppeteer@23.11.1 \
      n8n-nodes-puppeteer \
...
# 6) Whisper + PyTorch (GPU) + tiktoken + pre-download “base” model

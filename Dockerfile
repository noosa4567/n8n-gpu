...
# 1) Create non-root “node” user **with UID=999** to match your NAS
...
# 4) Install Node.js 20.x (with npm)
...
# 6) Install PyTorch/CUDA wheels, Whisper + tokenizer, pre-download “base” model
RUN pip3 install --no-cache-dir \
      --index-url https://download.pytorch.org/whl/cu118 \
      torch==2.1.0+cu118 numpy==1.26.3 \
 && pip3 install --no-cache-dir tiktoken openai-whisper==20231117 \

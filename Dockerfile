...
#── 6) Install Node 20, n8n & Puppeteer globally via NodeSource
...
#── 8) Chrome “warm-up” (run as root so NODE_PATH is in effect)
...
#── 13) Tiny PATH-shim so /usr/local/bin comes first (FIXED!)
RUN printf '%s\n' \
      '#!/bin/sh' \
